...
router.put("/v1/profileupdate/:uid", async (req, res) => {
  const { uid } = req.params;
  const { name, email, mobile, userprofile, location, desc } = req.body;
  ...
  try {
    ...
    existingProfile.userprofile = userprofile;
    existingProfile.location = location;
    existingProfile.desc = desc;
...
router.get("/v11/postdetail/:postid", async (req, res) => {
    ...
    try {
        ...
        const post = await ProfilePost.findOne({ uid: postid });
        if (!post) {
            return res.status(404).json({ message: "Post not found" });
        }
...
router.post("/v16/addtocart/:uid", async (req, res) => {
  const { uid } = req.params;
  const { productId, quantity } = req.body;

  if (!uid || !productId || !quantity) {
    return res.status(400).json({ message: "UID, Product ID, and Quantity are required" });
  }
  ...
  try {
    ...
    const product = await ProfilePost.findById(productId);

    if (!product) {
      return res.status(404).json({ message: "Product not found" });
    }

    profile.cart.push({ productId, quantity });
    await profile.save();

    res.status(200).json({ message: "Item added to cart successfully", cart: profile.cart });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
router.post("/v17/removefromcart/:uid/:productId", async (req, res) => {
  const { uid, productId } = req.params;

  if (!uid || !productId) {
    return res.status(400).json({ message: "UID and Product ID are required" });
  }
  ...
  try {
    ...
    const index = profile.cart.findIndex(item => item.productId.toString() === productId);
    ...
    res.status(200).json({ message: "Item removed from cart successfully", cart: profile.cart });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Internal Server Error" });
  }
